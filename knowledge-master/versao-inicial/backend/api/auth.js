...
module.exports = app => {
    ...
    const validateAdmin = async (req, res) => {
        ...
        try {
            if(userData) {
                const token = jwt.decode(userData.token, authSecret)
                const user = await app.db('users').where({ email: token.email }).whereNull('deletedAt').first()
            ...
            res.status(401).send(`${user.name} não tem permissão para acessar esta página`)
        }
